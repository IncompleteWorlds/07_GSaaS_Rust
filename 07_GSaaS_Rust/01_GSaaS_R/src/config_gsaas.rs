...
pub struct ConfigVariables {
    ...
    // External NNG address
    pub gsaas_nng_address:          String,

    // Internal main control address and port
    pub gsaas_int_address:          String,

    // External FDS NNG address and ports
    pub gsaas_nng_sub_address:      String,
}
...
impl ConfigVariables 
{
    pub fn new() -> Self 
    {
        ConfigVariables {
            ...
            gsaas_http_address:         String::new(),
            gsaas_nng_address:          String::new(),
            gsaas_int_address:          String::new(),
            gsaas_nng_sub_address:      String::new(),
        }
    }
}
...
impl ToString for ConfigVariables {
    fn to_string(&self) -> String {
        ...
        output_buffer.push_str( format!("GSaaS HTTP API Address: {}\n", self.gsaas_http_address).as_str() );
        output_buffer.push_str( format!("GSaaS NNG API Address: {}\n", self.gsaas_nng_address).as_str() );

        output_buffer.push_str( format!("Internal Control Address: {}\n", self.gsaas_int_address).as_str() );

        output_buffer.push_str( format!("FDS Bus Address: {}\n", self.gsaas_nng_sub_address).as_str() );
...
// TODO: If it is static, maybe it could be cloned. And there is no need to be global and static 
// lazy_static! {
