 ...
 * 
 * Ground Segment as a Service main
 * It will implement the entry point for the REST API and
 * Internal API
 * It will act as a Gateway API
    Log access
    Authenticate users
    Filter operations
...
// Actix Web Server
use actix_web::{rt::System, web, dev::Server, App, HttpResponse, HttpServer, Responder, HttpRequest /*middleware*/};
...
fn main_control_loop() 
{
    ...
    let unused_result = pub_control_socket.dial( tmp_config_data.gsaas_int_address.as_str() );
    ...
    info!("Correctly connected to Main Control (PUB). Address: {}", tmp_config_data.gsaas_int_address);
    ...
    // Start dialing
    let unused_result = sub_control_socket.listen( tmp_config_data.gsaas_nng_sub_address.as_str() );
    if let Err(e) = unused_result {
        error!("NNG Error when starting dialing the socket: {}", e );
        return;
    }
    info!("NNG Correctly connected to FDS SUB Address: {}", tmp_config_data.gsaas_nng_sub_address);
...
async fn get_version() -> impl Responder 
{
    ...
    let tmp_version = GetVersionResponseStruct{ version : GSAAS_VERSION.to_string() };
...
async fn api_usage(in_operation : web::Path<String>) -> impl Responder 
{
    ...
    match in_operation.as_str() {
        "register" => {
            usage_msg = "Register a new user";
        },

        "deregister" => {
            usage_msg = "Deregister or remove an existing user"
        },
 ...
 * Forward a message to the main control loop and wait for the answer
 */
async fn fds_forward_message(in_payload: String, in_request: HttpRequest, in_db: web::Data<DbPool>) -> impl Responder 
{
 ...
 * Register a new user
 */
async fn register_user(in_request: HttpRequest, in_db: web::Data<DbPool>) -> impl Responder 
{
    info!("TODO Register user: " );

    return HttpResponse::Ok()
                        .content_type("text/html")
                        .body( "TODO Register user" );
}

/**
 * Deregister/Delete an user
 */
async fn deregister_user(in_request: HttpRequest, in_db: web::Data<DbPool>) -> impl Responder 
{
    info!("TODO Deregister user: " );

    return HttpResponse::Ok()
                        .content_type("text/html")
                        .body( "TODO Deregister user" );
}

/**
 * Login
 */
async fn login(in_request: HttpRequest, in_db: web::Data<DbPool>) -> impl Responder 
{
    info!("TODO Login user: " );

    return HttpResponse::Ok()
                        .content_type("text/html")
                        .body( "TODO Login user" );
}

/**
 * Logout
 */
async fn logout(in_request: HttpRequest, in_db: web::Data<DbPool>) -> impl Responder 
{
    info!("TODO Logout user: " );

    return HttpResponse::Ok()
                        .content_type("text/html")
                        .body( "TODO Logout user" );
}
 ...
 * Forward a message to the main control loop and wait for the answer
 */
async fn do_exit(in_request: HttpRequest, in_db: web::Data<DbPool>) -> impl Responder 
{
    info!("TODO Exit server: " );
...
/**
 * Record the HTTP access into a DB table
 */
 fn record_access(in_request: &HttpRequest)
 {
    let tmp_conn;
    // Get the DB Pool
    {
        let tmp_global_data = GLOBAL_DATA.read().unwrap();
        tmp_conn =  tmp_global_data.db_pool.clone();
    }
...
fn usage() 
{
    ...
    println!("Ground Segment as a Service - HTTP API");
    println!("");
 ...
 */
fn run_app(tx: mpsc::Sender<Server>, in_http_address: String) -> std::io::Result<()> 
{
    ...
    let srv = HttpServer::new(move || {
        ...
        .service(
            web::scope("/gsaas")
                ...
                .route("/api/exit", web::get().to(do_exit))
                ...
                // USERS MANAGEMENT
                // ---------------------------------
                .route("/api/register", web::put().to(register_user))
                .route("/api/deregister", web::put().to(deregister_user))
            ...
            web::scope("/fdsaas")
                .default_service(
                    web::route().to(not_allowed_method),
                                    ...
                                    // GENERAL
                // ---------------------------------
                .route("/api/{operation}/usage", web::get().to(fds_forward_message))
                .route("/api/version", web::get().to(fds_forward_message))
                .route("/api/status", web::get().to(fds_forward_message))

        
                // PROPAGATE AN ORBIT
                // ---------------------------------
                .route("/api/orb_propagation", web::get().to(fds_forward_message))
                .route("/api/orb_propagation_tle", web::get().to(fds_forward_message))

                // Execute a plain GMAT script
                // ---------------------------------
                .route("/api/run_script", web::get().to(fds_forward_message))
...
async fn main() -> std::io::Result<()>
{
    ...
    fs::write("gsaas.pid", data).expect("Unable to write 'gsaas.pid' file");
    ...
    info!("External HTTP API Address: {}", config_variables.gsaas_http_address);

    // Connect to the GS server via Nanomsg 
    info!("External NNG API Address: {}", config_variables.gsaas_nng_address);
    ...
    // Data shared between all threads
    {
        ...
        tmp_data.gsaas_status    = EnumStatus::RUNNING;
        tmp_data.nng_ip_address  = config_variables.gsaas_int_address.clone();
    ...
    // Remove the pid file
    fs::remove_file("gsaas.pid").expect("Unable to remove 'gsaas.pid' file");
