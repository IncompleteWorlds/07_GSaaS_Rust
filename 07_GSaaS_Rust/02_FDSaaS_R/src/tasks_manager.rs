...
{   
    ...
    // Store a copy of the DB Pool
    pub fn set_db_pool(&mut self, in_pool: DbPool) {
        self.db_pool = Some(in_pool);
    }
     ...
     */
    pub fn add_task(&mut self, in_module_id : u32, in_instance_id: u32, in_user_id: String) -> Result<u32, String>
    {
        ...
        if self.db_pool.is_some() {
            let tmp_pool = self.db_pool.clone().unwrap();
            if let Err(e) = exec_record_db.create( &tmp_pool.get().unwrap() ) {
                let error_msg : String = format!("Error adding task record: {}", e);
                error!("{}", error_msg);
