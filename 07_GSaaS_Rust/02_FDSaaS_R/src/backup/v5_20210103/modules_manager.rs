...
impl ModuleManager {
     ...
     */
    pub fn call_module(&self, in_json_message: &RestRequest) -> Result<InternalResponseMessage, String> {
        info!("Processing message JSON: {}", in_json_message.to_string());

        let mut received_response: InternalResponseMessage = InternalResponseMessage::new_empty();
        let tmp_msg_code_id = in_json_message.msg_code_id.clone();
        let tmp_msg_id = in_json_message.msg_id.clone();

        // Get user id
        //let user_id = in_json_message.parameters.user_id.clone();
        let user_id : String;
        if in_json_message.parameters["user_id"].is_null() == false {
            match in_json_message.parameters["user_id"].as_str() {
        ...
        for (i, current_module) in tmp_data.list_running_modules.iter().enumerate() {
            if current_module.definition.messages.contains(&tmp_msg_code_id) == true
            {
                ...
                // Send messages
                let tmp_message = Message::from(&serde_json::to_vec(&new_json_message).unwrap());

                // Send the message to the main control loop of the module
                // Message is a JSON
                debug!("Sending message: {}", new_json_message.to_string());
                ...
                // Create response
                received_response = InternalResponseMessage::new_wait(tmp_msg_id.as_str(), task_id);
    ...
    */
     ...
     */
    pub fn handle_module_answer(&self, in_json_message: &RestRequest) -> Result<InternalResponseMessage, String> {
        ...
        let mut response_json_message = InternalResponseMessage::new_empty();

        response_json_message.msg_id = tmp_request.msg_id.clone();
        response_json_message.execution_id = tmp_execution_id;
        response_json_message.wait_flag = true;
        response_json_message.parameters = tmp_request.parameters.clone();
...

        {
            TASK_MANAGER.write().unwrap()
                .set_answer_completed(
                    tmp_execution_id,
                    tmp_request.parameters.to_string(),
                )
                .unwrap();
        }

        // Create response
        Ok(response_json_message)
    }
     ...
     */
    pub fn module_is_ready(&self, in_json_message: &RestRequest) -> Result<InternalResponseMessage, String> {
        ...
        if in_json_message.parameters["module_instance_id"].is_null() == true {
            let error_msg = format!("Module Get Status response does not contain the instance id");
            error!("{}", error_msg);
            return Err(error_msg);
        }

        let read_instance_id : u32 = in_json_message.parameters["module_instance_id"].as_u64().unwrap() as u32;

        println!("{}", in_json_message.parameters);

        if in_json_message.parameters["status"].is_null() == true {
            let error_msg = format!("Module Get Status response does not contain the status field");
            error!("{}", error_msg);
            return Err(error_msg);
        }

        let module_status = in_json_message.parameters["status"].as_str().unwrap();
        ...
        response_json_message.msg_id = in_json_message.msg_id.clone();
        response_json_message.execution_id = 0;
        response_json_message.wait_flag = false;
        response_json_message.parameters = Value::Null; //in_json_message.parameters.clone();
