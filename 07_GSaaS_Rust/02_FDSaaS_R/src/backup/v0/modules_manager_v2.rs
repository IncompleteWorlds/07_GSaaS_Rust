...
pub struct ModuleManager {
   // Private
   // ---------------------
   pub list_running_modules:      Rc< RefCell< Vec<Module> > >,
   pub list_executions:           Rc< RefCell< Vec<ExecutionRecord> > >,
   pub executions_counter:        Rc< RefCell< u32 > >,
...
impl ModuleManager
{
   ...
   {
      ModuleManager {
         list_running_modules:     Rc::new( RefCell::new( Vec::new() ) ),
         list_executions:          Rc::new( RefCell::new( Vec::new() ) ),
         executions_counter:       Rc::new( RefCell::new( 0 ) ),
      }
   }
    ...
    */
   pub async fn load_module_definitions(&mut self, in_config_variables: &ConfigVariables) -> std::result::Result<(), String> 
   {      
      ...
      for a_definition in tmp_list
      {
         ...
         self.list_running_modules.borrow_mut().push(new_module);
   ...
   pub async fn call_module(&mut self, in_json_message: &Value) -> Result<String, String> 
   {
      ...
      for (i, a_module) in self.list_running_modules.borrow().iter().enumerate() {
      ...
      // Get the module
      let tmp_list = Rc::clone(&self.list_running_modules);
      let mut tmp_list = tmp_list.borrow_mut();

      let tmp_a_module = tmp_list.get_mut(module_index);
...
/*
    ...
    * 
    */
   //pub async fn call_module_async(&mut self, in_json_message: &Value) -> Result<String, String>
   //{
      // info!("Processing message JSON: {}", in_json_message.to_string() );
      ...
      // for (i, current_module) in self.list_running_modules.iter_mut().enumerate() {
      ...
      // for current_module in Arc::get_mut(&mut self.list_running_modules).unwrap().iter_mut() {
      //    if current_module.definition.messages.contains( &tmp_msg_code_id ) == true {
      //       let a_module = current_module;
      //       module_found = true;
      //       break;
      //    }
      // }
      ...
      //          if let Err(e) = self.run_module1(current_module).await {
      //             return Err( build_api_answer_str_json(true, e.to_string().as_str(), "") );
      //          }
      //       }
    ...
    */
   pub async fn handle_module_answer(&mut self, in_json_message: &Value) -> Result<u32, String> 
   {
      ...
      // Get Execution record index
      for current_execution in self.list_executions.borrow_mut().iter_mut() {
    ...
    * If we do not execute child.status(), the process will be a zombie
      * wait
         ...
         kill
      */
   pub async fn check_status_all_modules(&mut self) 
   {
      for a_module in self.list_running_modules.borrow_mut().iter_mut() {      
   ...
   pub async fn kill_all_modules(&self)
   {
      ...
      for a_module in self.list_running_modules.borrow().iter() {
    ...
    * Add an execution record to the list
      ...
      */
   pub async fn add_execution_record(&mut self, in_module: &Module) -> u32
   {
      let current_time = Utc::now();

      // Next counter
      *self.executions_counter.borrow_mut() += 1;

      // Create a copy
      let current_counter : u32 = *self.executions_counter.borrow();
      ...
      self.list_executions.borrow_mut().push(exec_record);
   ...
   async fn get_execution_record(&self, in_execution_id: u32) -> u32
   {
      ...
      for (i, current_execution) in self.list_executions.borrow().iter().enumerate() {
   ...
   async fn remove_execution_record(&mut self, in_execution_index: u32)
   {
      ...
      if i < self.list_executions.borrow().len() {
         self.list_executions.borrow_mut().remove(i);
      }
   }
   ...
   async fn set_stop_time(&mut self, in_execution_index: u32)
   {
      let i = in_execution_index as usize;
      if i < self.list_executions.borrow().len() {
         self.list_executions.borrow_mut()[in_execution_index as usize].stop_time = format!("{}", Utc::now().naive_utc() );
      }
   }
    ...
    * Stop the async task
    */
   async fn wait_for_task(&mut self, in_execution_index: u32)
   {
      let i = in_execution_index as usize;
      if i < self.list_executions.borrow().len() {
         let ee = self.list_executions.clone();

         ee.borrow_mut()[in_execution_index as usize].wait_task = Some( WaitForAnswerFuture::new() );
         
         // It should block the task here
         //self.list_executions.borrow_mut()[in_execution_index as usize].wait_task = Some( t_task );
         
         // Wait for the task
         if let Some(t) = &mut ee.borrow_mut()[in_execution_index as usize].wait_task {
             t.await;
         };
      }
   }
   ...
   async fn get_received_answer(&self, in_execution_index: u32) -> String
   {
      let i = in_execution_index as usize;
      if i < self.list_executions.borrow().len() {
         return self.list_executions.borrow_mut()[in_execution_index as usize].answer.clone();
      } else {
         return String::from("");
      }
   }
