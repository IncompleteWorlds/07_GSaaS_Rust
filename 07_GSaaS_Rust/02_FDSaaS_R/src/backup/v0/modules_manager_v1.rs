...
static mut list_running_modules: Vec<Module> = Vec::new();
static  mut list_executions: Vec<ExecutionRecord> = Vec::new();
static  mut executions_counter:        u32 = 0;

pub fn module_manager_init() 
{
}
 ...
 *  Load the list of modules from the JSON config file
   */
pub async fn load_module_definitions(in_module_manager: &mut ModuleManager, in_config_variables: &ConfigVariables) -> std::result::Result<(), String> 
{      
   ...
   for a_definition in tmp_list
   {
      ...
      // Run the module
      let _tmp_result = run_module(&mut new_module).await;
      ...
      in_module_manager.list_running_modules.push(new_module);
 ...
 */
/*
pub async fn call_module(in_module_manager: &mut ModuleManager, in_json_message: &Value) -> Result<String, String> 
{
   ...
   for (i, a_module) in in_module_manager.list_running_modules.iter().enumerate() {
   ...
   // Get the module
   let mut tmp_a_module = in_module_manager.list_running_modules.get_mut(module_index);
   ...
   if a_module.status != EnumModuleStatus::RUNNING {
      ...
      if let Err(e) = run_module(a_module).await {
   ...
   // Next counter
   in_module_manager.executions_counter += 1;
   // Create a copy
   let current_counter = in_module_manager.executions_counter;
   ...
   let exec_record = ExecutionRecord {
      ...
      wait_task:             None, //WaitForAnswerFuture::new(),
   };

   // Save execution record
   debug!("Creating execution record {:?}", exec_record);
   in_module_manager.list_executions.push(exec_record);

   // Get Execution record index
   let mut execution_record_index = 0;

   for (i, current_execution) in in_module_manager.list_executions.iter().enumerate() {
 ...
 * 
 */
pub async fn call_module_async(in_module_manager: &mut ModuleManager, in_json_message: &Value) -> Result<String, String>
{
   info!("Processing message JSON: {}", in_json_message.to_string() );
   ...
   let a_module; // = in_module_manager.list_running_modules.iter_mut().next().unwrap();

   // for (i, current_module) in in_module_manager.list_running_modules.iter_mut().enumerate() {
   ...
   for current_module in &mut in_module_manager.list_running_modules {
      if current_module.definition.messages.contains( &tmp_msg_code_id ) == true {
         a_module = current_module;
         module_found = true;
         break;
      }
   }
   ...
   let current_counter = add_execution_record(in_module_manager, &a_module).await;
   

   // Get Execution record index
   let mut execution_record_index = get_execution_record(in_module_manager, current_counter).await;
   ...
   wait_for_task(in_module_manager, execution_record_index);
   debug!("Unblocked task");


   // We received the notification and the value has been updated, we can leave.
   received_response = get_received_answer(in_module_manager, execution_record_index).await;
   info!("Received response: {}", received_response);


   set_stop_time(in_module_manager, execution_record_index);

   // TODO: Save the Execution record in a Database

   // Remove execution from the list
   remove_execution_record(in_module_manager, execution_record_index);
...
/**
 * Handle answer from a module
 * It will receive the answer and wake up the dormant task
 */
pub async fn handle_module_answer(in_module_manager: &mut ModuleManager, in_json_message: &Value) -> Result<u32, String> 
{
   ...
   // Get Execution record index
   for current_execution in in_module_manager.list_executions.iter_mut() {
 ...
 * If we do not execute child.status(), the process will be a zombie
   * wait
      ...
      kill
   */
pub async fn check_status_all_modules(in_module_manager: &mut ModuleManager) 
{
   for a_module in in_module_manager.list_running_modules.iter_mut() {
      ...
      if let Err(e) = status {
         ...
         kill_module(a_module).await;

         // TODO: Remove instance from the module

         for _i in 0..2 {
            run_module(a_module).await;
 ...
 * Try to stop all modules. If not possible, kill them
   */
pub async fn kill_all_modules(in_module_manager: &ModuleManager)
{
   info!("Stopping all modules");

   for a_module in in_module_manager.list_running_modules.iter() {
 ...
 * Add an execution record to the list
   ...
   */
pub async fn add_execution_record(in_module_manager: &mut ModuleManager, in_module: &Module) -> u32
{
   let current_time = Utc::now();
   ...
   // Create an execution record
   let exec_record = ExecutionRecord {
      execution_id:          in_module_manager.executions_counter,
      // TODO
      // in_user_id
      user_id:               111,
 ...
 * Return an execution record based on its counter
   */
async fn get_execution_record(in_module_manager: &ModuleManager, in_execution_id: u32) -> u32
{
   let mut execution_record_index = 0;
 ...
 * Remove an execution record from the list
   */
async fn remove_execution_record(in_module_manager: &mut ModuleManager, in_execution_index: u32)
{
   let i = in_execution_index as usize;
   if i < in_module_manager.list_executions.len() {
      in_module_manager.list_executions.remove(i);
   }
}
 ...
 * Set the stop time of an execution
   */
async fn set_stop_time(in_module_manager: &mut ModuleManager, in_execution_index: u32)
{
   let i = in_execution_index as usize;
   if i < in_module_manager.list_executions.len() {
      in_module_manager.list_executions[in_execution_index as usize].stop_time = format!("{}", Utc::now().naive_utc() );
   }
}
 ...
 * Stop the async task
   */
async fn wait_for_task(in_module_manager: &mut ModuleManager, in_execution_index: u32)
{
   let i = in_execution_index as usize;
   if i < in_module_manager.list_executions.len() {
      // It should block the task here
      in_module_manager.list_executions[in_execution_index as usize].wait_task = Some( WaitForAnswerFuture::new() );
   }
}

async fn get_received_answer(in_module_manager: &ModuleManager, in_execution_index: u32) -> String
{
   let i = in_execution_index as usize;
   if i < in_module_manager.list_executions.len() {
      return in_module_manager.list_executions[in_execution_index as usize].answer.clone();
   } else {
      return String::from("");
   }
}
 ...
 */
async fn kill_module(in_module: &mut Module) 
{
...
*/
async fn run_module(in_module: &mut Module) -> Result<u32, String> 
{
