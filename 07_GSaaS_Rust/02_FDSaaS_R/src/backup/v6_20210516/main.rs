...
fn main_control_loop() 
{
    ...
    while done_flag == false {
        ...
        let mut processed_flag : bool = false;
        // let mut processing_result;
        //let mut response_json_message : InternalResponseMessage;
        //let mut tmp_msg_id : String;
        ...
        // If it is not a Request, maybe it is a response
        if processed_flag == false {
            let json_message = serde_json::from_slice(json_buffer.as_slice());
            //let _response_message : InternalResponseMessage = match json_message {
            let response_json_message : InternalResponseMessage = match json_message {
                Ok(msg1) => {
                    debug!("   ****** InternalResponseMessage ");
    
                    let processing_result = process_incoming_response(&msg1);
                    match processing_result {
                        Ok(r) => {
                            r
                        },
                        Err(e) => {
                            let tmp_msg = format!("ERROR: Processing InternalResponseMessage JSON message: {}. IGNORED", e.to_string());
                            error!("{}", tmp_msg.as_str() );
                            
                            InternalResponseMessage::new_error("error_response", msg1.response.msg_id.clone(), tmp_msg.as_str(), 0)
                        },
                    }
                },
                Err(e) => {
                    ...
                    //response_json_message = InternalResponseMessage::new_error("error", String::from("0"), tmp_msg.as_str(), 0);
                    InternalResponseMessage::new_error("error_response", String::from("0"), tmp_msg.as_str(), 0)
                },
            };
...
fn nng_control_loop() 
{
    ...
    while done_flag == false {
        ...
        if let Err(e) = input_msg {
            ...
            let tmp_main_socket = main_control_socket.clone();

            let r_thread = thread::spawn(move || {
                let _ = forward_message_internal(&tmp_main_socket, &payload);
            });
        ...
        if let Err(e) = input_msg {
            ...
            let r_thread = thread::spawn(move || {
                let output_msg = forward_message_internal(&tmp_main_socket, &payload);
 ...
 */
//async 
fn forward_message_internal(in_socket: &Socket, in_payload: &String) -> Result<String, String> 
{
