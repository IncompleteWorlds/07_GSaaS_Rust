...
impl User 
{
     ...
     */
    pub fn register(conn: &SqliteConnection, in_json_message: &RestRequest) -> Result<InternalResponseMessage, String> 
    {       
        // Decode JSON
        ...
        match User::create(conn, &register_message.username, &new_password, &register_message.email, &new_secret_key) {
            Ok(nu) => {
                ...
                let output = InternalResponseMessage::new(in_json_message.msg_id.as_str(), 
                                                          json!(tmp_user), 0);
                return Ok(output);
            },
            Err(e) => {
    ...
    */
    ...
    pub fn logout(_conn: &SqliteConnection, in_json_login: &RestRequest) -> Result<InternalResponseMessage, String> {
        let output = InternalResponseMessage::new(in_json_login.msg_id.as_str(), 
                                                  Value::Null, 
                                                  0);
        Ok(output)
    }

    pub fn login(conn: &SqliteConnection, in_json_login: &RestRequest) -> Result<InternalResponseMessage, String> {
        // Decode the JSON object
        let in_user : LoginStruct = match serde_json::from_value(in_json_login.parameters.clone()) {
            Ok(u) => u,
            Err(e) => {
