...
fn stop_all_loops(in_global_data : Arc<Mutex<GlobalData>>) 
{
    // Stop the other loops
    let mut tmp_data = GLOBAL_DATA.read().unwrap();
 ...
 * 
 */
async fn main_control_loop(in_config_variables: ConfigVariables, in_global_data : Arc<Mutex<GlobalData>>) 
{
    ...
    let rep_control_socket   = match rep_control_socket  {
        ...
        Err(e) => {
            ...
            stop_all_loops( in_global_data.clone() );
            return;
        },
    };

    // Start listening
    let unused_result = rep_control_socket.listen( in_config_variables.fds_int_req_address.as_str() );
    ...
    info!("Correctly connected to Main Control (REP). Address: {}", in_config_variables.fds_int_req_address);
    ...
    if let Err(_e) = ModuleManager::current().load_module_definitions(&in_config_variables).await {
    ...
    //     };
    // }
    let tmp_global_data = in_global_data.lock().unwrap();
 ...
 * 
 */
fn nng_control_loop(in_config_variables: ConfigVariables, in_global_data : Arc<Mutex<GlobalData>>) 
{
    ...
    // Start listening
    let unused_result = bus_control_socket.listen( in_config_variables.fds_nng_bus_address.as_str() );
    if let Err(e) = unused_result {
        error!("Error when starting listening the socket: {}", e );
        return;
    }
    info!("Correctly connected to FDS BUS. Address: {}", in_config_variables.fds_nng_bus_address);
    ...
    // Start listening
    let unused_result = rep_control_socket.listen( in_config_variables.fds_nng_rep_address.as_str() );
    if let Err(e) = unused_result {
        error!("Error when starting listening the socket: {}", e );
        return;
    }
    info!("Correctly connected to FDS REP. Address: {}", in_config_variables.fds_nng_rep_address);
    ...
    // Start listening
    let unused_result = sub_control_socket.listen( in_config_variables.fds_nng_sub_address.as_str() );
    if let Err(e) = unused_result {
        error!("Error when starting listening the socket: {}", e );
        return;
    }
    info!("Correctly connected to FDS SUB Address: {}", in_config_variables.fds_nng_sub_address);
    ...
    while done_flag == false {
        ...
        if let Err(e) = input_msg {
            ...
            block_on( forward_message_internal(&main_control_socket, &payload, in_global_data.clone()) ).unwrap();
        ...
        if let Err(e) = input_msg {
            ...
            let output_msg = block_on( forward_message_internal(&main_control_socket, &payload, in_global_data.clone()) );
 ...
 */
async fn get_status(in_global_data: web::Data<Arc<Mutex<GlobalData>>>) -> impl Responder 
{
    info!("   *** Get Status");
    ...
    let tmp_data = in_global_data.lock().unwrap();
 ...
 * 
 */
async fn forward_message_internal(in_socket: &Socket, in_payload: &String, in_global_data: Arc<Mutex<GlobalData>>) -> Result<String, String> 
{
    debug!("INT FORWARD sending message: {}", in_payload);

    // Create WaitFor task
    let new_wait_task = wait_for_task::WaitForAnswerFuture::new();
    ...
    let execution_id = TaskListManager::current().add_task( new_wait_task );
    ...
    // Wait for answer
    TaskListManager::current().wait_for_task(execution_id);
 ...
 * Forward a message to the main control loop and wait for the answer
 */
async fn forward_message(in_payload: String, in_global_data: Arc<Mutex<GlobalData>>) -> impl Responder 
{
    ...
    {
        let tmp_global_data = in_global_data.lock().unwrap();
        tmp_control_socket = tmp_global_data.nng_socket.clone();
    }
    ...
    let tmp_control_socket = match tmp_control_socket {
        ...
        None => { 
            ...
            // }
            {
                let mut tmp_global_data = in_global_data.lock().unwrap();
                tmp_global_data.nng_socket = Some( new_control_socket.clone() );
            }
    ...
    // Create a new task
    let http_output = forward_message_internal(&tmp_control_socket, &in_payload, in_global_data.clone() ).await;
...
async fn main() -> std::io::Result<()> 
{
    ...
    let nng_thread = thread::spawn(move || {
        nng_control_loop(tmp_config_variables1, nng_global_data);
    });

    
    // Main control loop thread
    //let main_thread = thread::spawn(|| { 
        main_control_loop(tmp_config_variables3, main_global_data).await;
    //}); 
